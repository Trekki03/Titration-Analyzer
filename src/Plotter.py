...
def plotAllData(xListTitration, yListTitration, mLin, bLin, xNullLin, xHighSlopeLin, xE, yE, highPointX):
    figure = plt.Figure(figsize=(5,4), dpi=100)
    ax = figure.add_subplot(1,1,1)
    ax.plot(xListTitration, yListTitration, color = "blue", marker = "x", label = "titration readings")
    linFuncValues = createLinValues(mLin, bLin, xNullLin, highPointX)
...
def createLinValues(m, b, xNull, xHighPoint):
    x = np.linspace(xNull, xHighPoint)
    y = (m*x) + b
    return (x,y)
